...
RSpec.describe NotePlan::QuickOpen do
  ...
  context "#alfred_items" do
    ...
    before do
      allow(Alfred::Item).to receive(:new).with("#foo", match: "foo").and_return(alfred_item_foo)

      allow(Alfred::Item).to receive(:new).with("#bar", match: "bar").and_return(alfred_item_bar)
